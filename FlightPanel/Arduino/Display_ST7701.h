...
#define ESP_PANEL_LCD_RGB_BOUNCE_BUF_SIZE         (20 * ESP_PANEL_LCD_WIDTH)      // Bounce buffer size in px. Doubled vs default to prevent drift when WiFi DMA competes.
                                                          // Must satisfy: (h_res * v_res) / size = even integer. 480*480/9600=24 (even) ✓
